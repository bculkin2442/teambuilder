...
/**
 * @author Nicholas Hauschild Date: 5/12/13 Time: 6:24 PM
 */
	...
	public HttpResponse invoke(final HttpRequest httpRequest) {
		...
		try {
			...
			for (Map.Entry<String, String> entry : httpRequest
				...
				queryParams.append("&").append(encodedKey).append("=")
						.append(encodedValue);
			}

			URL url = new URL(
					httpRequest.getEndpoint() + queryParams.toString());
			...
			if (httpRequest.getMethod().isBodySupported()) {
				...
				for (Map.Entry<String, String> entry : httpRequest
						.getBodyParams().entrySet()) {
					final String encodedKey = URLEncoder
							.encode(entry.getKey(), "UTF-8");
					final String encodedValue = URLEncoder
							.encode(entry.getValue(), "UTF-8");

					bodyParams.append(encodedKey).append("=")
							.append(encodedValue).append("&");
				}
				...
				conn.getOutputStream()
						.write(bodyParams.toString().getBytes());
			...
			HttpResponse respons = new HttpResponse(conn.getResponseCode(),
					response.toString().trim(), conn.getHeaderFields());
			...
			try {
				...
				switch (responseCode) {
					case 422:
						...
						final InputStream errorStream = conn
								.getErrorStream();
				...
				// couldn't get response code, so just wrap original
				// exception
				throw new RuntimeException(e);
			}
		} finally {
